...
internal class WorkbenchSettings : PersistentStateComponent<WorkbenchSettings.Data> {
    ...
    data class Preference(
        var root: String = "", var python: String = "python3", var query: String = "", var status: String = "未完成", var incompleteDefaultApplied: Boolean = false, var tab: Int = 0, var run: String = "", var item: String = "", var document: String = "", var positions: MutableList<Position> = mutableListOf(), var reviewHosts: MutableList<ReviewHost> = mutableListOf(),
    )
    data class WorkItemView(var slug: String = "", var tab: Int = 0, var changeTab: Int = 1, var filter: String = "全部", var task: String = "", var repository: String = "", var file: String = "", var offset: Int = 0, var startCommit: String = "")
    data class Binding(var projectRoot: String = "", var kitRoot: String = "")
    data class Data(var preferences: MutableList<Preference> = mutableListOf(), var bindings: MutableList<Binding> = mutableListOf(), var itemViews: MutableList<WorkItemViewEntry> = mutableListOf())
    data class WorkItemViewEntry(var root: String = "", var view: WorkItemView = WorkItemView())
    ...
    fun itemView(root: String, slug: String): WorkItemView {
        require(slug.isNotBlank())
        val key = canonical(root)
        return data.itemViews.firstOrNull { it.root == key && it.view.slug == slug }?.view
            ?: WorkItemView(slug = slug, tab = preference(root).tab.coerceIn(0, 2)).also {
                data.itemViews += WorkItemViewEntry(key, it)
            }
    }

    fun itemStatus(root: String): String = preference(root).let {
